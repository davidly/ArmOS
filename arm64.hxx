...
struct Arm64
{
...
#ifdef TARGET_BIG_ENDIAN
    uint64_t getui64( uint64_t o ) { uint64_t x; memcpy( &x, getmem( o ), sizeof( x ) ); return flip_endian64( x ); }
    uint32_t getui32( uint64_t o ) { uint32_t x; memcpy( &x, getmem( o ), sizeof( x ) ); return flip_endian32( x ); }
    uint16_t getui16( uint64_t o ) { uint16_t x; memcpy( &x, getmem( o ), sizeof( x ) ); return flip_endian16( x ); }
    float getfloat( uint64_t o ) { uint32_t x = getui32( o ); float f; memcpy( &f, &x, sizeof( f ) ); return f; }
    double getdouble( uint64_t o ) { uint64_t x = getui64( o ); double d; memcpy( &d, &x, sizeof( d ) ); return d; }

    void setui64( uint64_t o, uint64_t val ) { val = flip_endian64( val ); memcpy( getmem( o ), &val, sizeof( val ) ); }
    void setui32( uint64_t o, uint32_t val ) { val = flip_endian32( val ); memcpy( getmem( o ), &val, sizeof( val ) ); }
    void setui16( uint64_t o, uint16_t val ) { val = flip_endian16( val ); memcpy( getmem( o ), &val, sizeof( val ) ); }
    void setfloat( uint64_t o, float val ) { uint32_t x; memcpy( &x, &val, sizeof( x ) ); setui32( o, x ); }
    void setdouble( uint64_t o, double val ) { uint64_t x; memcpy( &x, &val, sizeof( x ) ); setui64( o, x ); }
#else
    uint64_t getui64( uint64_t o ) { uint64_t x; memcpy( &x, getmem( o ), sizeof( x ) ); return x; }
    uint32_t getui32( uint64_t o ) { uint32_t x; memcpy( &x, getmem( o ), sizeof( x ) ); return x; }
    uint16_t getui16( uint64_t o ) { uint16_t x; memcpy( &x, getmem( o ), sizeof( x ) ); return x; }
    float getfloat( uint64_t o ) { float x; memcpy( &x, getmem( o ), sizeof( x ) ); return x; }
    double getdouble( uint64_t o ) { double x; memcpy( &x, getmem( o ), sizeof( x ) ); return x; }

    void setui64( uint64_t o, uint64_t val ) { memcpy( getmem( o ), &val, sizeof( val ) ); }
    void setui32( uint64_t o, uint32_t val ) { memcpy( getmem( o ), &val, sizeof( val ) ); }
    void setui16( uint64_t o, uint16_t val ) { memcpy( getmem( o ), &val, sizeof( val ) ); }
    void setfloat( uint64_t o, float val ) { memcpy( getmem( o ), &val, sizeof( val ) ); }
    void setdouble( uint64_t o, double val ) { memcpy( getmem( o ), &val, sizeof( val ) ); }
  ...
  private:
    ...
    template <typename T> T round_int_from_double( double d, uint64_t fracbits, FPRounding rounding );
